...
```{r}
library(tidyverse)
CHOL1b <- CHOL1a %>% gather(`4`:`8`, key = 'Week', value = 'Response')
CHOL1b$Week <- as.numeric(CHOL1b$Week)
str(CHOL1b)
```
...
```{r}
CHOL.COMBINED <- merge(CHOL2a, CHOL1b, by.x = 'Brand', by.y = 'Margarine')
head(CHOL.COMBINED)
```
...
```{r}
names(S_VS_D)[1] <- 'State'
VIOLENT2 <- anti_join(VIOLENT, S_VS_D, by = 'State')
str(VIOLENT2)
```
...
VIOLENT3 <- VIOLENT2
VIOLENT3 <-mutate_if(VIOLENT3, is.character,str_replace_all,pattern ="Louisville Metro", replacement = "Louisville")
VIOLENT3 <-mutate_if(VIOLENT3, is.character,str_replace_all,pattern ="Nashville Metropolitan", replacement = "Nashville")
VIOLENT3 <-mutate_if(VIOLENT3, is.character,str_replace_all,pattern ="Washington", replacement = "Washington Dc")
names(CENSUS)[1] <- 'City'

VIOLENT4 <- merge(VIOLENT3, CENSUS, by = c('State', 'City'), all.x = T)
str(VIOLENT4)

```

Answer *(2 Points)*: (Place Answer Here Using Complete Sentences) 
Lexington is missing values for the state.y, change and density values in this instance of the table of violent 4.
...
```{r}
dr  = merge(ZIP,STATE_ABBREV, by = 'state')
dr = select(dr,subset= -c("state"))
names(dr)[1] = 'City'

FINAL.VIOLENT = merge(VIOLENT4,dr, by = c('State', 'City'),all.x = TRUE)
str(FINAL.VIOLENT)
```
```{r}
FINAL.VIOLENT
...
Answer *(1 Points)*: (Place Answer Here Using Complete Sentences) 
 Wahsington dc is missing the value for the longitude and latitude since the state abbreviation for this was shown as columbia, where as this wasn't originally shown in the census which makes this not show up.
...
Answer *(2 Points)*: (Place Answer Here Using Complete Sentences) 
The table I plan on scraping is the list of countries by population. It contains variables realted to the population in 2018 and 2019. There is also a variable which contains the change over the past year in anopther varaible.This table is from the united nations and gives us a list of all the countries while also providing the region that each of the countries on the list is from
...
data <- read_html("https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations)") %>% html_table(fill = T)
