...
for(i in 1:7){
  for(j in 1:7){
    ...
    DATA4.PREDICT = DATA4 %>% 
          ...
          select(predict) %>%
          unnest()
   OUT.RMSE[i,j] = RMSE.func(actual=DATA4.PREDICT$W,predict=DATA4.PREDICT$.fitted)
...
Code and Output:
```{r,eval=T}
BEST5.RMSE = OUT.RMSE2[order(OUT.RMSE2$RMSE),]
BEST5.RMSE = as.tibble(BEST5.RMSE[1:5,])
...
BEST5.DATA2 <- BEST5.DATA %>%
  gather(key ="Model", value = "Predict" ,6:10, factor_key = T)
...
```{r,eval=T}
ggplot(BEST5.DATA2) +
    geom_point(aes(x =D,y =W ,alpha = 0.05, stroke = 0)) + geom_line( aes(x =D,y =Predict,color = Model))
```
...
This graph indicates that as the degree of the polynomial increases the lower the error is. However we can see that for a degree 4 polynomial the error is the lowest for both I and J. However what we know is that as the degree increases a lot we are overfitting thus decreasing the reliability of such predictions due to the high reliance on the training model although the accuracy increases.The one reccomeded for I is 4. While for J we see that 4 would also be good since there isn't much further change in the error after and you don't want to overfit as mentioned earlier. 
...
BEST.func=function(data){
  OUT.RMSE=matrix(NA,7,7) #DO NOT CHANGE
    BEST.CHOICE= data[order(data$RMSE),]
    BEST.CHOICE= head(BEST.CHOICE,1)
    BEST.CHOICE = as.numeric((c(BEST.CHOICE[1],BEST.CHOICE[2])))
    return(BEST.CHOICE)
}
...
Code and Output:
```{r,eval=T}
BEST.MODEL = lm(W~poly(A,3) + poly (D,6), data = DATA)

DATA4 <- DATA %>%
  mutate(fitted_values = predict(BEST.MODEL))

ggplot(DATA4, mapping = aes(x = W, y = fitted_values)) + geom_point() + xlab("Actual Max Water Temperature") + ylab("Fitted Max Water Temperature") + geom_abline(slope = .75, intercept = 3, color = "red")
```
...
```{r,eval=T}
DATA5 = DATA %>%
  mutate(res = residuals(BEST.MODEL))

DATA5 %>% ggplot() + geom_point(mapping = aes(x = D, y = res)) + geom_hline(yintercept = 0, color = "red", linetype = 2) + xlab("Day") + ylab("Residual")
```